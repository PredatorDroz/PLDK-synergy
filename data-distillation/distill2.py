...
def main(args):
    ...
    wandb.init(sync_tensorboard=False,
               project="DatasetDistillation_mod",
               job_type="Repo",
                entity='',
               config=args,
               )
        ...
        if it in eval_it_pool:
            for model_eval in model_eval_pool:
                ...
                if acc_test_mean > best_acc[model_eval]:
                    ...
                    save_this_it = True
                    torch.save(net_eval.state_dict(), os.path.join('D:/research_2022/mtt-distillation-main/mtt-distillation-main/data/GEN_A/', 'net_conv_%acc_test_mean.pth' % acc_test_mean))
        ...
        if it in eval_it_pool and (save_this_it or it % 1000 == 0):
            with torch.no_grad():
                ...
                if args.ipc < 50 or args.force_save:
                    ...
                    #wandb.log({"Syn": wandb.Image(torch.nan_to_num(upsampled[0].detach().cpu()))}, step=it)
                    j=0
                    for i in range(100):
                        if(i%10==0):
                            j+=1
                        ...
                        img = upsampled[i]
                        strg='D:/research_2022/mtt-distillation-main/mtt-distillation-main/data/GEN_A/img'+str(j)+"_"+str(i)+'.png'
                        save_image(img, strg)


                    samples = upsampled1.cpu()
                    data_x.append(copy.deepcopy(samples.cpu()))
                    data_y.append(copy.deepcopy(label_syn.cpu()))

                    data_x = np.concatenate(data_x)
                    data_x = np.transpose(data_x, [0, 2, 3, 1])
                    data_y = np.concatenate(data_y)
                    name="D:/research_2022/mtt-distillation-main/mtt-distillation-main/data/GEN_A/gen_data_"+str(it)
                    np.savez_compressed(name, data_x=data_x, data_y=data_y)
                    ...
                    if args.zca:
                        ...
                        for i in range(100):
                            ...
                            img = upsampled[i]
                            strg='D:/research_2022/mtt-distillation-main/mtt-distillation-main/data/GEN_AC/img'+str(j)+"_"+str(i)+'.png'
                            save_image(img, strg)
                        ...
                        data_x = np.transpose(data_x, [0, 2, 3, 1])
                        data_y = np.concatenate(data_y)
                        name="D:/research_2022/mtt-distillation-main/mtt-distillation-main/data/GEN_AC/gen_data_recon"+str(it)
                        np.savez_compressed(name, data_x=data_x, data_y=data_y)
